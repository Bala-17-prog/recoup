    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-blue-500/30 pb-20 relative">
      <header className="bg-slate-900/50 backdrop-blur-xl border-b border-white/10 px-4 md:px-8 py-4 md:py-5 flex flex-col md:flex-row items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex flex-wrap items-center gap-4 md:gap-6 justify-center w-full md:w-auto">
          <div className="flex items-center gap-2 bg-slate-900/80 p-1.5 rounded-xl border border-white/5 ring-1 ring-inset ring-white/5">
            <button 
              onClick={() => {
                  setDataSource('demo');
                  setShowUploadFlow(false);
      ...
      <main className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
        {showUploadFlow ? (
            <UploadFlow onComplete={handleUploadComplete} onCancel={() => setShowUploadFlow(false)} />
        ) : (
            <>
          ...
          <div className="bg-gradient-to-br from-indigo-900/40 via-slate-900 to-slate-900 rounded-3xl border border-white/10 shadow-2xl p-6 md:p-10 relative overflow-hidden mb-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-slate-950/50 rounded-2xl p-6 border border-red-500/10 relative overflow-hidden">
                      ...
                      <div className="space-y-4">
                          <div>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total At Risk</p>
                  ...
                  <div className="bg-gradient-to-br from-emerald-950/40 to-slate-900 rounded-2xl p-6 border border-emerald-500/20 relative overflow-hidden">
                      ...
                      <div className="space-y-4">
                          <div>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Projected Recovered Value</p>
                              <p className="text-3xl font-black text-emerald-400">₹{formatIN(metrics?.total_recovered)}</p>
                          </div>
                          <div className="flex justify-between items-center border-t border-white/5 pt-3">
                              <div>
                                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Remaining At Risk</p>
                                  <p className="text-sm font-bold text-slate-300">₹{formatIN((metrics?.total_at_risk || 0) - (metrics?.total_recovered || 0))}</p>
                              </div>
        ...
        <div className="bg-slate-900/40 rounded-2xl border border-white/10 shadow-xl overflow-hidden flex flex-col">
            ...
            <div className="overflow-x-auto max-h-[300px] custom-scrollbar">
                <table className="w-full text-left text-sm min-w-[900px] whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {audit.filter(log => !!log.promise_date && log.promise_date.trim() !== "" && log.promise_date !== "NaN").length === 0 ? (
                            <tr>
                                <td colSpan="6" className="px-6 py-12 text-center text-slate-500 font-mono">No promises found in current view.</td>
                            </tr>
                        ) : audit.filter(log => !!log.promise_date && log.promise_date.trim() !== "" && log.promise_date !== "NaN").map((log, i) => (
        ...
        <div className="bg-slate-900/40 rounded-2xl border border-white/10 shadow-xl overflow-hidden">
          ...
          <div className="overflow-x-auto max-h-[600px] custom-scrollbar">
            <table className="w-full text-left text-sm whitespace-nowrap min-w-[1500px]">
              ...
              <tbody className="divide-y divide-white/5">
                {audit.length === 0 ? (
                    <tr>
                        <td colSpan="11" className="px-5 py-12 text-center text-slate-500 font-mono">No audit trail records found.</td>
                    </tr>
                ) : audit.map((log) => {
                  ...
                  <tr key={log.id} className="hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-5 py-4 align-top">
                        <button 
                            ...
                            className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-[10px] uppercase font-bold text-blue-400 rounded transition-colors tracking-wider whitespace-nowrap border border-slate-700"
                        >
                            View Details
                        </button>
                    </td>
